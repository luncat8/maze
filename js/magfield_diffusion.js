// Diffusion magnetic-field solver — the Hy3 screened-Poisson engine.
//
...
// OWN field. magDiffusionPublish subtracts each magnet's own dipole field
...
// into `dst`. Blocked / out-of-bounds neighbours are treated as 0 (no-flux
// Dirichlet), so the field does not leak through walls and decays naturally
// outside the wires.
...
function magDiffusionBuildSource() {
	...
	for (let ei = 0; ei < fieldEdges.length; ei++) {
		...
		if (e.dlx === 1) {
			// horizontal edge: A=(ax,ay), B=(bx,ay) with bx=ax+1
			magSrcHy3[ay * W + ax] += I;        // bottom edge of A (+)
			magSrcHy3[by * W + bx] -= I;        // top edge of B (−)
		} else {
			// vertical edge: A=(ax,ay), C=(ax,by) with by=ay+1
			magSrcHy3[ay * W + ax] += I;        // right edge of A (+)
			magSrcHy3[by * W + bx] -= I;        // left edge of C (−)
		}
	}
